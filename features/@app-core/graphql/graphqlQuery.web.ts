...
export const graphqlQuery = Object.assign(
  async <T extends TadaDocumentNode, R = ResultOf<T>>(
    query: T,
    config?: QueryConfig<T>
  ) => {
    // Config
    const { variables, headers, graphqlEndpoint, requestContext } =
      config || {};
    ...
    if (isServer) {
      try {
        ...
        const [{ graphql }, { executableSchema }] = await Promise.all([
          import('graphql'),
          import('./schema'),
        ]);
