...
export const CheckListContext = createContext<CheckListContext | undefined>(
  undefined
);
...
export const CheckListOptionProps = CheckboxProps.omit({
  checked: true,
...
export const CheckListOption = (props: CheckListOptionProps) => {
  // Props
  const { value, label, ...restProps } =
    CheckListOptionProps.applyDefaults(props);
...
export const CheckListProps = CheckboxProps.omit({
  checked: true,
  label: true,
...
export type CheckListProps<T extends string[] = string[]> = z.input<
  typeof CheckListProps
> & {
  value: T;
  children?: ReactNode;
...
const CheckListComponent = <T extends string[] = string[]>(
  rawProps: CheckListProps<T>
) => {
  // Props
  ...
  const toggleValue = (value: string) => {
    ...
    setValues(
      shouldAdd ? [...currentItems, value] : values.filter((v) => v !== value)
    );
  };
  ...
  useEffect(() => {
    if (valuesKey !== propValuesKey) {
      setValues(currentItems);
    }
  }, [propValuesKey, currentItems, valuesKey]);
...
export const getDocumentationProps = CheckListProps.documentationProps(
  'CheckList',
  {
    exampleProps: {
      value: ['web'],
