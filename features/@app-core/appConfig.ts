import {
  createDriverConfig,
  DRIVER_OPTIONS,
} from '@app/registries/drivers.config';
...
export const isNext =
  (!isExpo && isWeb) || process.env.NEXT_PUBLIC_APP_ENV === 'next' || isDocs;

export const isWebLocalServer =
  isWeb && isServer && process.env.PORT === '3000';
export const isWebLocalhost =
  isWeb && (globalThis?.location?.hostname === 'localhost' || isWebLocalServer);
export const isExpoWebLocal =
  isWebLocalhost && globalThis?.location?.port === '8081';
...
export const expoDebuggerHost =
  Constants?.expoGoConfig?.debuggerHost ||
  Constants.manifest2?.extra?.expoGo?.debuggerHost; // prettier-ignore
export const localURL =
  expoDebuggerHost?.split?.(':').shift() || fallbackExpoWebHost;
...
export const baseURL =
  ...
  `${fallbackBaseURL}`; // prettier-ignore
...
export const apiURL =
  ...
  `${fallbackBaseURL}/api`; // prettier-ignore
export const graphURL =
  process.env.NEXT_PUBLIC_GRAPH_URL ||
  process.env.EXPO_PUBLIC_GRAPH_URL ||
  `${fallbackBaseURL}/api/graphql`; // prettier-ignore
...
export const isMobileSize = isServer
  ? undefined
...
export const appConfig = {
  ...
  appDescription:
    Constants?.manifest2?.description || Constants?.manifest?.description,
