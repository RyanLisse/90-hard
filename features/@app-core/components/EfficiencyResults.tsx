...
export const EfficiencyResults = (props: EfficiencyResultsProps) => {
  ...
  const { shipsWebOnly, shipsMobileOnly, shipsUniversal, isProvider, isDev } =
    results;
  const { showEfficiencyBoost, showValueDelivered, showRepositioningBenefits } =
    results;
  const { annualAvgEfficiencyBoost, annualHoursSaved } = results;
  const { deliveryEfficiency, finalEfficiencyRate } = results;
  const { learningGapHours, learningGapDecimals, setupHoursPerProject } =
    results;
  const { projects, identity, benefitLevel, beneficial, convincee, handover } =
    results;
    ...
    <>
      ...
      <H1 className="text-5xl text-primary">
        ...
        {showEfficiencyBoost ? (
          ...
          <Text className="text-warn">{'slightly '}</Text>
        )}
        {'more efficiently'}
      </H1>
      ...
      <View className="flex flex-row items-center">
        <Button
          ...
          size="sm"
          text={shipsMobileOnly ? '❇️  + Web' : '✅  Web'}
        ...
        <Button
          className={cn('cursor-default', shipsWebOnly && 'border-success')}
          size="sm"
          text={shipsWebOnly ? '❇️  + Android' : '✅  Android'}
          textClassName={cn(shipsWebOnly && 'font-bold')}
          type="outline"
        />
      </View>
      ...
      <H3 className="text-secondary">Team Knowledge?</H3>
      ...
      <View className="flex-row">
        <Button
          ...
          fullWidth
          iconLeft={showBenefits ? undefined : 'AddFilled'}
          ...
          type={
            showBenefits || !formState.values.identifiesWith
              ? 'secondary'
              : 'success'
          }
        />
      ...
      {showBenefits && (
        <>
          ...
          <P className="text-muted">
            {'As a '}
            <Text className="font-bold text-primary">{identity}</Text>
          ...
          {showRepositioningBenefits && (
            <>
              ...
              <P className="text-muted">
                ...
                {' grants you '}
                <Text className="font-bold text-primary">more flexibility</Text>
              ...
              {!shipsUniversal && (
                <Li>{'Charge higher for more platforms delivered'}</Li>
              )}
          ...
          {!shipsUniversal && (
            <>
              ...
              <Li>
                {'More platforms = '}
                <Text className="italic">more trust</Text>
              </Li>
              ...
              <Li>
                {'Your app will have '}
                <Text className="font-bold">universal deeplinks</Text>
              </Li>
          ...
          <Li>
            <Link
              href="https://fullproduct.dev/docs/data-fetching"
              target="_blank"
            >
          ...
          <Li>
            <Link
              href="https://fullproduct.dev/docs/write-once-styles"
              target="_blank"
            >
              {'Style universal UI'}
            </Link>
          ...
          <Li>
            <Link
              href="https://fullproduct.dev/docs/single-sources-of-truth"
              target="_blank"
            >
          ...
          <Li>
            <Link
              href="https://fullproduct.dev/docs/data-resolvers"
              target="_blank"
            >
          ...
          {isProvider && (
            <>
              ...
              <Li>
                <Link
                  href="https://fullproduct.dev/docs/project-structure"
                  target="_blank"
                >
          ...
          <P className="text-muted">
            {'This starterkit can '}
            <Text className="font-bold text-primary">{'generate docs'}</Text>
          ...
          <P className="text-muted">
            {'Check out some '}
            <Link
              href="https://fullproduct.dev/docs/@app-core/components/Button"
              target="_blank"
            >
          ...
          {isDev && (
            <P className="text-muted">
              ...
              <Link
                href="https://fullproduct.dev/docs/single-sources-of-truth#building-schemas-with-zod"
                target="_blank"
              >
                {'Zod schemas'}
              </Link>
