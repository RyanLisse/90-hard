...
import type { DRIVER_KEYS } from '@app/registries/drivers.config';
...
export const healthCheck = createResolver(
  async ({ args, withDefaults, req, context }) => {
    // Inputs
    ...
    const extraContext = {
      requestContext: headerContext,
      drivers: {},
    } as ObjectType<any$Unknown>;
    ...
    try {
      const hasDbDriverSetting = !!(
        await Settings.findMany({ key: 'dbDriver' })
      ).length;
