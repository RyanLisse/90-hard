...
import {
  ...
  globRel,
  parseWorkspaces,
} from '@green-stack/scripts/helpers/scriptUtils';
...
const collectModels = () => {
  try {
    ...
    const modelRegistry = allModelPaths.reduce((acc, modelFilePath) => {
      ...
      const isValidModel = modelFileContents.includes(
        'export const driverModel = validateDriverModel('
      );
      if (!isValidModel) {
        return acc;
      }
      ...
      const modelFileModuleName = modelFilename
        .replace('.tsx', '')
        .replace('.ts', '');
      ...
      return {
        ...
        modelAliasEntryLines: addSetItem(
          acc.modelAliasEntryLines,
          modelModuleAliasLine
        ),
      };
    }, {} as ModelRegistry);
