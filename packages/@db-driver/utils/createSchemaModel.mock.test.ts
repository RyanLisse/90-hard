...
test("Model.driver.insertOne() throws if inserted record does not match schema", async () => {
  ...
  await expect(
    UserModel.driver.insertOne({ name: "Thorr", email: "" }),
  ).rejects.toThrow(); // Missing email
  await expect(
    UserModel.driver.insertOne({ name: "Thorr", email: "thorr@codinsonn" }),
...
test("Model.driver.insertMany() throws if inserted records do not match schema", async () => {
  ...
  await expect(
    UserModel.driver.insertMany([{ name: "Thorr" }]),
  ).rejects.toThrow(); // Missing required field email
  await expect(
    UserModel.driver.insertMany([{ name: "Thorr", email: "" }]),
  ).rejects.toThrow(); // Missing email
  await expect(
    UserModel.driver.insertMany([{ name: "Thorr", email: "thorr@codinsonn" }]),
  ).rejects.toThrow(); // Invalid email
});
...
test("Model.driver.findOne() finds a record stored in memory", async () => {
  ...
  const foundByEmail = await UserModel.driver.findOne({
    email: "thorr@codinsonn.dev",
  });
...
test("Model.driver.findMany() finds multiple records stored in memory", async () => {
  ...
  const _newUsers = await UserModel.driver.insertMany([
    {
      name: "Thorr",
      email: "thorr@codinsonn.dev",
    },
...
test("Model.driver.updateOne() updates a record stored in memory", async () => {
  ...
  const updatedUser = await UserModel.driver.updateOne(query, {
    email: "thorr@fullproduct.dev",
  });
...
test("Model.driver.updateOne() with errorOnUnmatched: true throws if updated record does not match schema", async () => {
  ...
  expect(() =>
    UserModel.driver.updateOne({ id: newUser.id }, {}),
  ).not.toThrow(); // Should be fine
  expect(() =>
    UserModel.driver.updateOne({ id: newUser.id }, { name: "Thor" }),
  ).not.toThrow(); // Missing required field email
  // @ts-expect-error
  expect(() =>
    UserModel.driver.updateOne(
      { id: newUser.id },
  ...
  expect(() =>
    UserModel.driver.updateOne({ id: newUser.id }, { name: undefined }, true),
  ).toThrow(); // Not nullable or optional
  expect(() =>
    UserModel.driver.updateOne(
      { id: newUser.id },
      { name: "Thorr", email: "" },
...
test("Model.driver.updateMany() updates multiple records stored in memory", async () => {
  ...
  const updatedUsers = await UserModel.driver.updateMany(query, {
    name: "Thor",
  });
...
test("Model.driver.updateMany() with errorOnUnmatched: true throws if updated records do not match schema", async () => {
  ...
  expect(() =>
    UserModel.driver.updateMany({ name: "Thorr" }, { name: undefined }, true),
  ).toThrow(); // Not nullable or optional
  expect(() =>
    UserModel.driver.updateMany(
      { name: "Thorr" },
      { name: "Thorr", email: "" },
      true,
    ),
  ...
  expect(() => UserModel.driver.updateMany({}, {})).not.toThrow(); // Should be fine
  expect(() =>
    UserModel.driver.updateMany({ name: "Thorr" }, { name: "Thor" }),
  ).not.toThrow(); // Should be fine
  // Check that we don't throw an error when updating records that don't exist
  expect(() =>
    UserModel.driver.updateMany({ email: "" }, { name: "Thorr" }),
  ).not.toThrow();
});
...
test("Model.driver.upsertOne() updates an existing record if it exists", async () => {
  ...
  const notFoundByEmail = await UserModel.driver.findOne({
    email: "thorr@codinsonn.dev",
  });
...
test("Model.driver.deleteOne() deletes a record stored in memory", async () => {
  ...
  const deletedUser = await UserModel.driver.deleteOne({
    email: "thorr@codinsonn.dev",
  });
  ...
  expect(() => UserModel.driver.deleteOne({ email: "" })).not.toThrow();
});
...
test("Model.driver.deleteMany() deletes multiple records stored in memory", async () => {
  ...
  expect(() => UserModel.driver.deleteMany({ email: "" })).not.toThrow();
});
