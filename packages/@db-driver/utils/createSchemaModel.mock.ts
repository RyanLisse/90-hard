...
import type { z } from '@green-stack/schemas';
...
} from './createSchemaModel.types.ts';

/** --- createSchemaModel() -------------------------------------------------------------------- */
/** -i- Creates a schema model to interface with the mock memory DB */
export const createSchemaModel = <
  S extends z.ZodRawShape,
  DataType extends Prettify<z.input<z.ZodObject<S>>> = Prettify<
    z.input<z.ZodObject<S>>
  >, // prettier-ignore
  QueryFilter extends QueryFilterType<Partial<DataType>> = QueryFilterType<
    Partial<DataType>
  >, // prettier-ignore
>(
  ...
  const idFields = arrFromSet([
    ...Object.keys(schemaMeta.schema!).filter((k) => {
  ...
  const softDeleteField = Object.keys(schemaMeta.schema!).find((key) => {
    if (!includesAny(key, ['deleted', 'removed'])) {
      return false;
    }
    return schemaMeta.schema?.[key].zodType === 'ZodDate';
  });
  ...
  const matchesCondition = (
    ...
    condition: QueryFilter[keyof QueryFilter]
  ): boolean => {
    ...
    if (isValidCondition) {
      return Object.entries(condition).every(([operator, operatorValue]) => {
        ...
        if (operator === '$in') {
          return operatorValue.includes(value);
        }
        if (operator === '$nin') {
          return !operatorValue.includes(value);
        }
        throw new Error(
          `Unsupported condition operator in query: ${JSON.stringify(condition)}`
        );
      });
    }
  ...
  const handleLogicalOperators = (
    ...
    switch (operator) {
      case '$and':
        return conditions.every((condition: QueryFilter) =>
          matchesQuery(record, condition)
        );
      case '$or':
        return conditions.some((condition: QueryFilter) =>
          matchesQuery(record, condition)
        );
      case '$nor':
        return !conditions.some((condition: QueryFilter) =>
  ...
  const matchesQuery = (record: DataType, query: QueryFilter): boolean => {
    return Object.entries(query).every(([key, value]) => {
      ...
      const isFieldOperator =
        isNestedQuery &&
        Object.keys(queryValue).some((operator) => {
  ...
  const insertOne = async (record: Partial<DataType>) => {
    ...
    try {
      ...
      if (memoryDB[modelKey]?.[id]) {
        throw new Error(
          `Record with ID "${id}" already exists in model "${modelKey}"`
        );
      }
      ...
      throw new Error(
        `Failed to insert record into "${modelKey}" collection: ${error.message}`
      ); // prettier-ignore
    }
  };
  ...
  const insertMany = async (records: Partial<DataType>[]) => {
    ...
    try {
      ...
      newRecords.forEach((record) => {
        if (memoryDB[modelKey]?.[record.id]) {
          throw new Error(
            `Record with ID "${record.id}" already exists in model "${modelKey}"`
          ); // prettier-ignore
        }
      });
      ...
      throw new Error(
        `Failed to insert records into "${modelKey}" collection: ${error.message}`
      ); // prettier-ignore
    }
  };
  ...
  const findOne = async (query: QueryFilter) => {
    ...
    try {
      ...
      throw new Error(
        `Failed to find record in "${modelKey}" collection: ${error.message}`
      );
    }
  };
  ...
  const findMany = async (query: QueryFilter) => {
    ...
    try {
      ...
      throw new Error(
        `Failed to find records in "${modelKey}" collection: ${error.message}`
      );
    }
  };
  ...
  const findOrCreate = async (
    ...
    try {
      ...
      throw new Error(
        `Failed to find or create record in "${modelKey}" collection: ${error.message}`
      );
    }
  };
  ...
  const updateOne = async (
    ...
    try {
      ...
      if (recordToUpdate.id !== updatedRecord.id) {
        ...
        throw new Error(
          `Cannot update a record's ID. Error while updating record with ID ${recordToUpdate.id} in "${modelKey}" collection`
        ); // prettier-ignore
      }
      ...
      if (recordId) {
        throw new Error(
          `Error while updating record with ID ${recordId} in "${modelKey}" collection: ${error.message}`
        );
      }
      throw new Error(
        `Failed to update record in "${modelKey}" collection: ${error.message}`
      ); // prettier-ignore
    }
  };
  ...
  const updateMany = async (
    ...
    try {
      ...
      if (!recordsToUpdate.length && errorOnUnmatched) {
        throw new Error(
          `No records found to update in "${modelKey}" collection`
        );
      }
      ...
      const updatedRecords = recordsToUpdate.map((record) => {
        ...
        if (record.id !== updatedRecord.id) {
          throw new Error(
            `Cannot update a record's ID. Error while updating record with ID ${record.id} in "${modelKey}" collection`
          ); // prettier-ignore
        }
        // Update the record in the memory DB
        memoryDB[modelKey][updatedRecord.id as string] = updatedRecord;
      ...
      throw new Error(
        `Failed to update records in "${modelKey}" collection: ${error.message}`
      );
    }
  };
  ...
  const upsertOne = async (query: QueryFilter, record: Partial<DataType>) => {
    ...
    try {
      ...
      if (recordToUpdate) {
        ...
        return updateOne(
          query,
          record as Prettify<Partial<Omit<DataType, 'id'>>>,
          true
        );
      }
      // Insert the record
      ...
      throw new Error(
        `Failed to upsert record in "${modelKey}" collection: ${error.message}`
      );
    }
  };
  ...
  const deleteOne = async (query: QueryFilter, errorOnUnmatched = false) => {
    ...
    try {
      ...
      if (!recordToDelete && errorOnUnmatched) {
        throw new Error(
          `No record found to delete in "${modelKey}" collection`
        );
      }
      if (!recordToDelete) {
      ...
      throw new Error(
        `Failed to delete record from "${modelKey}" collection: ${error.message}`
      );
    }
  };
  ...
  const deleteMany = async (query: QueryFilter, errorOnUnmatched = false) => {
    ...
    try {
      ...
      if (!recordsToDelete.length && errorOnUnmatched) {
        throw new Error(
          `No records found to delete in "${modelKey}" collection`
        );
      }
      ...
      throw new Error(
        `Failed to delete records from "${modelKey}" collection: ${error.message}`
      );
    }
  };
