  ...
  },
  {
    ...
    parser: (answers) => {
      ...
      const workspaceFolder =
        WORKSPACE_FOLDER_MAPPER[
          workspaceType as keyof typeof WORKSPACE_FOLDER_MAPPER
        ]; // prettier-ignore
      const workspacePath = `${workspaceFolder}/${folderName}`;
      const isFeature = workspaceType === 'feature';

      const usesCustomLicense = ['green-stack'].some((word) =>
        packageName.includes(word)
      );
      let packageLicense = isFeature ? 'MIT' : 'UNLICENSED';
      if (usesCustomLicense) {
        packageLicense = 'SEE LICENSE IN LICENSE.md';
      }
      ...
      const actions = [
        {
          ...
          templateFile:
            '../../packages/@green-stack-core/generators/templates/package-json.hbs',
          data: ctx,
        },
        {
          type: 'add',
