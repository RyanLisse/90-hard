...
>(
  ...
  const {
    validateOnBlur,
    validateOnChange,
    syncFromPropsKey = 'none',
  } = options;
  const initialValues = (options.initialValues || {}) as T;
  const _initialValuesKey = createKey(initialValues);
  ...
  const validate = (showErrors = true) => {
    ...
    const shouldUpdateErrors =
      !(validationError || isEmpty(errors)) && showErrors;
  ...
  const getErrors = <KEY extends K>(key: KEY) =>
  ...
  const getInputProps = <KEY extends K, VAL extends T[KEY] = T[KEY]>(
    key: KEY
  ) => ({
    value: values[key],
  ...
  const getNumberTextInputProps = <KEY extends K>(key: KEY) => {
    ...
    return {
      ...
      onChangeText: (value = '') => {
        ...
        if (!strippedValue) {
          return onChange(undefined as T[KEY]);
        }
        // Convert to number
  ...
  }, [initialState, schema.applyDefaults]);
