...
type ExtractPrimitives<T> = {
  [K in keyof T]: NonNullableRequired<T[K]> extends Primitive
    ? { [P in K]: T[K] }
    : ExtractPrimitives<T[K]>;
}[keyof NonNullableRequired<T>];

type DeepFlattenRequired<T> = {
  [K in keyof T]-?: NonNullableRequired<T[K]> extends Primitive
    ? NonNullableRequired<T[K]>
    : DeepFlattenRequired<NonNullableRequired<T[K]>>;
};
...
type ExtractRouteParams<T> = Partial<
  MergeUnion<ExtractPrimitives<DeepFlattenRequired<T>>>
>;
...
> = T extends undefined
  ? any$Ignore
  : Prettify<Partial<Pick<T, PICKED>> | Pick<T, PICKED>>;
