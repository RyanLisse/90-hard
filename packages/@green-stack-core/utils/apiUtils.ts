...
export const getApiParam = (key: string, apiSources: APISources) => {
  ...
  const [result] = [params, query, body, args, context]
    .map((src) => src?.[key])
    .filter(Boolean);
  return result;
};
 ...
 ** https://stackoverflow.com/a/63594903/8789673 */
export const firePostAndForget = (
  url: string,
  data: unknown,
  config?: RequestInit
) => {
...
export const encrypt = (data: string, secret = process.env.APP_SECRET) => {
  if (!secret) {
    throw new Error('Custom or APP_SECRET is required to encrypt data');
  }
...
export const decrypt = (data: string, secret = process.env.APP_SECRET) => {
  if (!secret) {
    throw new Error('Custom or APP_SECRET is required to decrypt data');
  }
...
export const createMiddlewareContext = async (
  ...
  if (!APP_SECRET) {
    warnOnce(
      'APP_SECRET variable is required to create header context, skipping context generation until it is set'
    ); // prettier-ignore
  }
  if (!req?.headers) {
    throw new Error(
      'Request headers are required to create header context, skipping context generation until they are available'
    ); // prettier-ignore
  }
...
export const getHeaderContext = (
  ...
  try {
    ...
    const headerContextRaw =
      requestContext || apiRequestContext || gqlRequestContext;
    if (!headerContextRaw) {
      return {};
    }
    // Parse context
    const headerContextJSON = Array.isArray(headerContextRaw)
      ? headerContextRaw[0]
      : headerContextRaw; // prettier-ignore
    const headerContext = JSON.parse(headerContextJSON);
    if (isEmpty(headerContext)) {
      return {};
    }
