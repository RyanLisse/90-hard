  ...
  Object.keys(objToValidate).map((propKey) => {
    ...
    const parseVal = (val: any$Unknown) => {
      ...
      // Normalize stringified numbers to actual number
        ...
        val = Array.isArray(val) || !isValidNumber(val) ? val : +val;
      }
      // Keep Dates as is
      ...
      // Keep Buffers as is
      if (typeof window === 'undefined' && val instanceof Buffer) {
        return val;
      }
    ...
    if (shouldIgnore) {
      val = objToValidate[propKey];
    } else {
      setProperty(obj, propKey, val);
    }
  });
  return obj;
};
...
export const swapEntries = <T extends EnumLike>(
  obj: T
): { [K in keyof T as T[K]]: K } => {
  const swapped: { [K in keyof T as T[K]]: K } = {} as {
    [K in keyof T as T[K]]: K;
  };
