...
export const parseGlobalCSS = (globalCSS: string) => {
  ...
  const processSection = (sectionText: string, themeKey: 'light' | 'dark') => {
    ...
    lines.forEach((line) => {
      const key = line.match(/--[\w-]+/)?.[0]; // Extract the variable name
      ...
      if (value.match(/^\d+,\s*\d+(?:\.\d+)?%,\s*\d+(?:\.\d+)?%$/)) {
        // Check for hsl pattern without 'hsl' prefix
        value = `hsl(${value})`;
      } else if (value.match(/^#([A-Fa-f0-9]{6}|[A-Fa-f0-9]{3})$/)) {
        // Check for hex pattern
        // value stays the same
      } else if (value.match(/^(rgb|rgba)\(/)) {
        // Check for rgb or rgba pattern
        // value stays the same
      }
  ...
  const rootSection = globalCSS
    .substring(rootIndex, darkRootIndex)
    .replace(':root {', '');
  let darkRootSection = globalCSS
    .substring(darkRootIndex)
    .replace('.dark:root {', '');
