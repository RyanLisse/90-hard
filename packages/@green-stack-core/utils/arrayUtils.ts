...
export const arrFromSet = <T = any$Unknown>(arr: T[] = []): T[] =>
  Array.from(new Set(arr));

/** --- addSetItem() --------------------------------------------------------------------------- */
/** -i- Adds an item to array if it doens't exist already */
export const addSetItem = <T = any$Unknown>(arr: T[], item: T): T[] => {
  if (!arr.map((itm) => JSON.stringify(itm)).includes(JSON.stringify(item))) {
    arr.push(item);
  }
  return arr;
};
...
>(
  ...
  return array.reduce(
    (lookup, current) => {
      ...
      if (!keyValue) {
        return lookup; // Skip if the item doesn't have a value for the key
      }
