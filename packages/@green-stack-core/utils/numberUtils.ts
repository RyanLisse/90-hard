...
export const roundUpTo = (value: number | string, decimals = 0) =>
  roundTo(value, decimals, ceil);
export const roundDownTo = (value: number | string, decimals = 0) =>
  roundTo(value, decimals, floor);
...
export const isValidNumber = (numCandidate: any): boolean => {
  ...
  const isNonNumberFalsy = ![true, false, null].includes(
    numCandidate as unknown as boolean
  );
  ...
  const canBeOperatedOn =
    isNonNumberFalsy && isNotArray && isNotNaN && doesntHaveLeadingZeroes;
