...
export const useScrollToFocusedInput = () => {
  ...
  // Vars
  type KeyboardBehaviour = ComponentProps<
  ...
  const handleScroll = (e: any$Ignore) =>
    (scrollYRef.current = e.nativeEvent.contentOffset.y);

  const handleFocusInput = (inputRef: MutableRefObject<TextInput | null>) => {
    return (_nativeEvent: any$Ignore) => {
      inputRef?.current?.measure((_x, _y, _w, h, _px, py) => {
  ...
  }, [isKeyboardVisible, height.set, keyboardPadding]);

  const keyboardPaddedView =
    Platform.OS !== 'web' ? (
      <Animated.View
        style={[
          {
