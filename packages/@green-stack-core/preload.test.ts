...
relevantTsConfigs.forEach(async (tsConfigPath) => {
  ...
  Object.entries(tsConfigPathAliases).forEach(([aliasKey, [aliasedPath]]) => {
    ...
    if (!aliasKey.includes('*')) {
      mock.module(aliasKey, () => import(aliasedPath));
    }
    // Wildcard * aliases to multiple files
    if (aliasKey.includes('*')) {
      const [importPathBase] = aliasedPath.split('*');
      const aliasGlob = aliasedPath
        .replace('*', '**/*.{ts,tsx}')
        .replace('../../', './');
      const aliasedPaths = globRel(aliasGlob);
      aliasedPaths.forEach((file) => {
        const actualFilePath = file
          .replace('./', '../../')
          .replace('.tsx', '')
          .replace('.ts', ''); // prettier-ignore
        const modulePathFromAlias = actualFilePath
          .split(importPathBase)[1]
          ?.replace('.tsx', '')
          .replace('.ts', ''); // prettier-ignore
        const normalizedAliasKey = aliasKey
          .replace('/*', '/')
          .replace('./', '../../');
...
mock.module(
  ...
  () => import('./__mocks__/expo-constants.mock')
); // @ts-ignore
