...
export const ZOD_TO_DEF = {
  // - Primitives -
  ZodString: (_fieldMeta: Metadata) => '.string()',
  ZodNumber: (_fieldMeta: Metadata) => '.number()',
  ZodBoolean: (_fieldMeta: Metadata) => '.boolean()',
  ZodDate: (_fieldMeta: Metadata) => '.date()',
  // - Advanced & Objectlikes -
  ZodEnum: (_fieldMeta: Metadata, innerDef = '...') => `.enum([${innerDef}])`,
  ZodArray: (_fieldMeta: Metadata, innerDef = '...') => `.array(${innerDef})`,
  ZodObject: (_fieldMeta: Metadata, innerDef = '...') =>
    `.object({\n${innerDef}\n})`,
  // - Mostly Supported, Experimental -
  ZodNull: (_fieldMeta: Metadata) => '.null()',
  ZodUndefined: (_fieldMeta: Metadata) => '.undefined()',
  ZodTuple: (_fieldMeta: Metadata, innerDef = '..., ...') =>
    `.tuple([${innerDef}])`,
  ZodUnion: (_fieldMeta: Metadata, innerDef = '..., ...') =>
    `.union([${innerDef}])`,
  ZodLiteral: (_fieldMeta: Metadata, innerDef = '...') =>
    `.literal(${innerDef})`,
  ZodNativeEnum: (_fieldMeta: Metadata, innerDef = '...') =>
    `.nativeEnum(${innerDef})`,
  // - Might Work, Not Advised -
  ZodAny: (_fieldMeta: Metadata) => '.any()',
  ZodRecord: (_fieldMeta: Metadata, innerDef = '...') => `.record(${innerDef})`,
  ZodUnknown: (_fieldMeta: Metadata) => '.unknown()',
  ZodBigInt: (_fieldMeta: Metadata) => '.bigint()',
  ZodSymbol: (_fieldMeta: Metadata) => '.symbol()',
  ZodIntersection: (_fieldMeta: Metadata, innerDef = '...') =>
    `.intersection(${innerDef})`,
  ZodDiscriminatedUnion: (_fieldMeta: Metadata, innerDef = '...') =>
    `.discriminatedUnion(${innerDef})`,
  ZodMap: (_fieldMeta: Metadata, innerDef = '...') => `.map(${innerDef})`,
  ZodSet: (_fieldMeta: Metadata, innerDef = '...') => `.set(${innerDef})`,
  // - Unsupported, Avoid in Schemas -
  ZodVoid: (_fieldMeta: Metadata) => '.void()',
  ZodFunction: (_fieldMeta: Metadata) => '.function()',
  ZodPromise: (_fieldMeta: Metadata) => '.promise()',
  ZodLazy: (_fieldMeta: Metadata, innerDef = '...') =>
    `.lazy(() => ${innerDef})`,
  ZodEffects: (_fieldMeta: Metadata, innerDef = '...') =>
    `.effects(() => ${innerDef})`,
} as const;
...
declare module 'zod' {
  ...
  > {
    ...
    extendSchema<S extends z.ZodRawShape>(
      name: string,
      shape: S
    ): ZodObject<T & S, UnknownKeys, Catchall>;
    ...
    documentationProps<P extends Input = Input, N extends string = string>(
      componentName: N,
    ...
    extend<S extends z.ZodRawShape>(
      shape: S
    ): ZodObject<T & S, UnknownKeys, Catchall>;
...
if (!ZodType.prototype.metadata) {
  ...
  const getStackedMeta = <Z extends z.ZodTypeAny>(
    ...
    if (zodStruct._def.defaultValue) {
      meta.defaultValue = zodStruct._def.defaultValue();
    }
    if (meta.defaultValue instanceof Set) {
      meta.defaultValue = Array.from(meta.defaultValue);
    }
    if (meta.exampleValue instanceof Set) {
      meta.exampleValue = Array.from(meta.exampleValue);
    }
    if (meta.defaultValue instanceof Map) {
      meta.defaultValue = Object.fromEntries(meta.defaultValue);
    }
    if (meta.exampleValue instanceof Map) {
      meta.exampleValue = Object.fromEntries(meta.exampleValue);
    }
    // Add the description if there is one
    if (zodStruct._def.description) {
      meta.description = zodStruct._def.description;
    }
    // Add array metadata if present
    if (zodStruct._def.minLength) {
      meta.minLength = zodStruct._def.minLength.value;
    }
    if (zodStruct._def.maxLength) {
      meta.maxLength = zodStruct._def.maxLength.value;
    }
    if (zodStruct._def.exactLength) {
      meta.exactLength = zodStruct._def.exactLength.value;
    }
    // Add string metadata if present
    const stringType = zodStruct as unknown as z.ZodString;
    if (stringType.minLength) {
      meta.minLength = stringType.minLength;
    }
    if (stringType.maxLength) {
      meta.maxLength = stringType.maxLength;
    }
    if (stringType.isBase64) {
      meta.isBase64 = true;
    }
    if (stringType.isEmail) {
      meta.isEmail = true;
    }
    if (stringType.isURL) {
      meta.isURL = true;
    }
    if (stringType.isUUID) {
      meta.isUUID = true;
    }
    if (stringType.isDate) {
      meta.isDate = true;
    }
    if (stringType.isDatetime) {
      meta.isDatetime = true;
    }
    if (stringType.isTime) {
      meta.isTime = true;
    }
    if (stringType.isIP) {
      meta.isIP = true;
    ...
    if (numberType.minValue) {
      meta.minValue = numberType.minValue;
    }
    if (numberType.maxValue) {
      meta.maxValue = numberType.maxValue;
    }
    if (numberType.isInt) {
      meta.isInt = numberType.isInt;
    }
    // Literals
    if (zodType === 'ZodLiteral') {
      ...
      if (typeof meta.literalValue === 'string') {
        meta.baseType = 'String';
      }
      if (typeof meta.literalValue === 'number') {
        meta.baseType = 'Number';
      }
      if (typeof meta.literalValue === 'boolean') {
    ...
    if (zodType === 'ZodEnum') {
      ...
      meta.schema = _inputOptions.options?.reduce(
        (acc: Record<string, unknown>, value: any) => {
    ...
    if (zodType === 'ZodTuple') {
      ...
      meta.schema = _zodTuple.items
        .map((item: any) => item.introspect?.(includeZodStruct))
        .filter(Boolean);
    }
    // Unions
    if (zodType === 'ZodUnion') {
      const _zodUnion = zodStruct as unknown as z.ZodUnion<any>;
      meta.schema = _zodUnion.options
        .map((option: any) => option.introspect?.(includeZodStruct))
        .filter(Boolean);
    }
    // Intersections
    if (zodType === 'ZodIntersection') {
      const _zodIntersection = zodStruct as unknown as z.ZodIntersection<
        any,
        any
      >;
      meta.schema = {
    ...
    if (zodType === 'ZodDiscriminatedUnion') {
      const _zodUnion = zodStruct as unknown as z.ZodDiscriminatedUnion<
        any,
        any
      >;
      meta.schema = _zodUnion.options.reduce(
        (acc: any, option: any) => {
          return {
            ...acc,
            types: [...acc.types, option.introspect?.(includeZodStruct)],
          };
        },
    ...
    if (zodType === 'ZodObject') {
      ...
      meta.schema = Object.entries(_zodObject.shape).reduce(
        (acc, [key, fieldType]) => {
          // @ts-expect-error
    ...
    if (isInnerMostStruct && includeZodStruct) {
      meta.innerStruct = zodStruct;
    }
  ...
  ZodType.prototype.introspect = function (includeZodStruct = false) {
    ...
    const zodType = innermostMeta.zodStruct?._def
      .typeName as unknown as ZOD_TYPE;
    ...
    // @ts-expect-error
    if (meta.literalBase) {
      meta.baseType = meta.literalBase;
    }
  ...
  ZodObject.prototype.applyDefaults = function <
    D extends Partial<(typeof thisSchema)['_input']> &
      Record<string, any$Unknown>,
  >(data: D, options: ApplyDefaultsOptions = {}) {
    const {
      logErrors = false,
      stripUnknown = false,
    ...
    const defaultValues = Object.keys(introSpectionResult.schema!).reduce(
      (acc, key) => {
        const fieldMeta = introSpectionResult.schema?.[key] as Metadata;
        const defaultValue = applyExamples
          ? fieldMeta.exampleValue
    ...
    if (stripUnknown) {
      ...
      const validData = Object.fromEntries(
        Object.entries(values).filter(([key]) => validKeys.includes(key))
      );
  ...
  ZodObject.prototype.documentationProps = function (
    ...
    return {
      ...
      previewProps: this.applyDefaults(config.exampleProps || {}, {
        applyExamples: true,
      }),
    };
  };
...
export const inputOptions = <T extends Readonly<Record<string, string>>>(
  obj: T
) => {
  ...
  return Object.assign(reassigned, { entries: obj }) as typeof zEnum & {
    entries: T;
  } & {
    [K in keyof T]: T[K];
  };
...
export const renderFieldMetaToZodDefV3 = (
  fieldKey: string,
  fieldMeta: Metadata
) => {
  // Extract metadata
  ...
  const getBaseDef = (key: ZOD_TYPE) =>
    ZOD_TO_DEF[key as keyof typeof ZOD_TO_DEF];
  const toBaseDef = getBaseDef(zodType as ZOD_TYPE);
  if (typeof toBaseDef !== 'function') {
    return '';
  }
  ...
  let innerDef = '';
  if (zodType === 'ZodEnum') {
    innerDef = Object.keys(fieldMeta.schema!)
      .map((key) => JSON.stringify(key))
      .join(', '); // @ts-ignore
  }
  if (zodType === 'ZodArray' && !fieldMeta.schema?.name) {
    innerDef = `z${getBaseDef(fieldMeta.schema?.zodType)?.(fieldMeta.schema)}`; // @ts-ignore
  }
  if (zodType === 'ZodArray' && fieldMeta.schema?.name) {
    innerDef = fieldMeta.schema?.name; // @ts-ignore
  }
  if (zodType === 'ZodObject' && fieldMeta.schema?.name) {
    innerDef = fieldMeta.schema?.name; // @ts-ignore
  }
  if (zodType === 'ZodObject' && !fieldMeta.schema?.name) {
    innerDef = '...'; // TODO: Fix this?
  }
  if (zodType === 'ZodTuple') {
    innerDef = fieldMeta.schema
      ?.map((entry: Meta$Tuple) => `z${getBaseDef(entry.zodType)?.(entry)}`)
      .join(', ');
  }
  if (zodType === 'ZodUnion') {
    innerDef = fieldMeta.schema
      ?.map((entry: Meta$Union) => `z${getBaseDef(entry.zodType)?.(entry)}`)
      .join(', ');
  }
  if (zodType === 'ZodLiteral') {
    innerDef = JSON.stringify(fieldMeta.literalValue); // @ts-ignore
  }
  if (zodType === 'ZodRecord' && fieldMeta.schema?.name) {
    innerDef = `z.string(), ${fieldMeta.schema?.name}`; // @ts-ignore
  }
  if (zodType === 'ZodRecord' && !fieldMeta.schema?.name) {
    innerDef = `z.string(), z${getBaseDef(fieldMeta.schema?.zodType)?.(fieldMeta.schema!)}`; // @ts-ignore
  }

  // Build the base definition for unproblematic types
  const baseDefV3 = toBaseDef(fieldMeta, innerDef);
  ...
  if (fieldMeta.isOptional) {
    optionality = `${tabs}.optional()`;
  }
  if (fieldMeta.isNullable) {
    optionality = `${tabs}.nullable()`;
  }
  if (fieldMeta.isNullable && fieldMeta.isOptional) {
    optionality = `${tabs}.nullish()`;
  }
  if (fieldMeta.defaultValue) {
    optionality = `${tabs}.default(${JSON.stringify(fieldMeta.defaultValue)})`;
  }
  ...
  const fieldEntry = [
    ...
    optionality,
    fieldMeta.exampleValue &&
      `${tabs}.example(${JSON.stringify(fieldMeta.exampleValue)})`,
    fieldMeta.description &&
      `${tabs}.describe(${JSON.stringify(fieldMeta.description)})`,
    fieldMeta.isSensitive &&
      `${tabs}.sensitive() // = stripped in API responses, serverside only`,
  ]
    .filter(Boolean)
    .join('\n');
...
export const renderSchemaToZodDefV3 = (
  schemaMeta: Meta$Schema,
  flavor: 'ZOD' | 'FPD' = 'ZOD'
) => {
  ...
  const fieldDefs = Object.entries(schemaMeta.schema!).map(
    ([fieldKey, fieldMeta]) => {
  ...
  return [
    flavor === 'FPD' &&
      `const ${schemaMeta.name} = schema('${schemaMeta.name}', {`,
