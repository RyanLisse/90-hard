...
import { type Meta$Schema, schema, z } from "../index";
...
test("Supports custom error messages for each validation step", () => {
  const ErrorTest = schema("ErrorTest", {
    ...
    num: z
      .number()
      .min(1, { message: "Too low" })
      .max(50, { message: "Too high" }),
  });
  // Default Error Messages
  expect(() => ErrorTest.shape.num.parse("")).toThrow(ZodError);
  expect(() => ErrorTest.shape.str.parse(0)).toThrow(ZodError);
  
  // Verify the error structure for type errors
  try {
    ErrorTest.shape.num.parse("");
  } catch (error) {
...
test("Primitives z.string(), z.number(), z.boolean() & z.date() work as expected", () => {
  ...
  expect(Primitives.shape.date.parse(new Date("2020-01-01"))).toEqual(
    new Date("2020-01-01"),
  );
...
test("Advanced types z.enum(), z.tuple(), z.union() & z.array() work as expected", () => {
  ...
  // Array
  expect(metadata.schema?.array.schema).toEqual({
    zodType: "ZodString",
    baseType: "String",
  });
...
test("Nested schemas work as expected", () => {
  ...
  // Parsing Happy Paths
  expect(Nested.shape.user.parse({ name: "John", age: 42 })).toEqual({
    name: "John",
    age: 42,
  });
  expect(
    Nested.shape.primitives.parse({
      str: "Hello",
      num: 42,
      bln: true,
