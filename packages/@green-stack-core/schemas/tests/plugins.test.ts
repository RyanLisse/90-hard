...
test('createSchemaPlugin() applies the correct builder pattern for baseTypes', () => {
  ...
  const testPlugin = createSchemaPlugin(UserSchema.introspect(), {
    Boolean: (_schemaKey, _fieldMeta) => 'BaseBool',
    String: (_schemaKey, _fieldMeta) => 'BaseString',
    Number: (_schemaKey, _fieldMeta) => 'BaseNumber',
    Date: (_schemaKey, _fieldMeta) => 'BaseDate',
  });
  // Check the result
...
test('createSchemaPlugin() overrides baseType builders for zodType builders', () => {
  ...
  const testPlugin = createSchemaPlugin(UserSchema.introspect(), {
    ...
    ZodBoolean: (_schemaKey, _fieldMeta) => 'ZodBool',
    ZodString: (_schemaKey, _fieldMeta) => 'ZodString',
    ZodNumber: (_schemaKey, _fieldMeta) => 'ZodNumber',
    ZodDate: (_schemaKey, _fieldMeta) => 'ZodDate',
  });
  // Check the result
...
test('createSchemaPlugin() overrides zodType builder for schemaKey builders', () => {
  ...
  const testPlugin = createSchemaPlugin(UserSchema.introspect(), {
    ...
    name: (_schemaKey, _fieldMeta) => 'KeyString',
    email: (_schemaKey, _fieldMeta) => 'KeyString',
    age: (_schemaKey, _fieldMeta) => 'KeyNumber',
    birthdate: (_schemaKey, _fieldMeta) => 'KeyDate',
  });
  // Check the result
...
test('createSchemaPlugin() builder functions recieve schemaKey as the first arg', () => {
  // Schema transformer to test
  const testPlugin = createSchemaPlugin(UserSchema.introspect(), {
    name: (schemaKey, _fieldMeta) => schemaKey,
    email: (schemaKey, _fieldMeta) => schemaKey,
    age: (schemaKey, _fieldMeta) => schemaKey,
    birthdate: (schemaKey, _fieldMeta) => schemaKey,
  });
  // Check the result
...
test('createSchemaPlugin() builder functions recieve fieldMeta as the second arg', () => {
  // Schema transformer to test
  const testPlugin = createSchemaPlugin(UserSchema.introspect(), {
    name: (_schemaKey, fieldMeta) => JSON.stringify(fieldMeta),
    email: (_schemaKey, fieldMeta) => JSON.stringify(fieldMeta),
    age: (_schemaKey, fieldMeta) => JSON.stringify(fieldMeta),
    birthdate: (_schemaKey, fieldMeta) => JSON.stringify(fieldMeta),
  });
  // Check the result
  expect(testPlugin.name).toBe(
    JSON.stringify(UserSchema.shape.name.introspect())
  );
  expect(testPlugin.email).toBe(
    JSON.stringify(UserSchema.shape.email.introspect())
  );
  expect(testPlugin.age).toBe(
    JSON.stringify(UserSchema.shape.age.introspect())
  );
  expect(testPlugin.birthdate).toBe(
    JSON.stringify(UserSchema.shape.birthdate.introspect())
  );
});
...
test('createGraphSchemaDefs() correctly splits resolvers into queries & mutations', () => {
  ...
  expect(Object.keys(mutationOnly.generatedResolvers?.Mutation)).toContain(
    'updateUser'
  );
  ...
  expect(Object.keys(both.generatedResolvers?.Mutation)).toContain(
    'updateUser'
  );
});
...
test('createGraphSchemaDefs() generates correct Query { .. } & Mutation { ... } definitions', () => {
  ...
  expect(printedSchema).toContain(
    'updateUser(args: UserSchemaInput!): UserSchema'
  );
});
