...
export const bridgedFetcher = <
  ...
  FinalQuery extends TadaDocumentNode | unknown = CustomQuery extends null
    ? BridgeQuery
    : CustomQuery,
>({
  allowedMethods,
  ...
  if (!(allowedMethods?.includes('GRAPHQL') && hasGraphqlQuery)) {
    throw new Error(
      'bridgedFetcher() with incompatible bridge. Please add "GRAPHQL" to the allowedMethods array in the DataBridge.'
    );
  }
  ...
  if (customQuery) {
    ...
    }) as typeof fetcher & { isUniversalQuery: true };
  }
