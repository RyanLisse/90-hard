...
import type { ApplyDefaultsOptions, z } from './index';
...
export type ResolverConfigType = {
  ...
  allowFail?: boolean;
  onError?: (err: any) => unknown | undefined;
  [key: string]: any;
};
...
export type ResolverExecutionParamsType<
  ArgsInput = any,
  ResOutput = any,
  ResInput = any,
> = {
...
export const createResolver = <
  ...
  ArgsInput = ArgsOverride extends null
    ? z.ZodObject<ArgsShape>['_input']
    : ArgsOverride,
  ResOutput = ResOverride extends null
    ? z.ZodObject<ResShape>['_output']
    : ResOverride,
  ResInput = ResOverride extends null
    ? z.ZodObject<ResShape>['_input']
    : ResOverride,
>(
  resolverFn: (
    ctx: ResolverExecutionParamsType<ArgsInput, ResOutput, ResInput>
  ) => Promise<ResOutput | unknown>,
  options: {
    paramKeys?: string;
  ...
  const resolverWrapper = (
    ctx?: ResolverInputType<ArgsInput>
  ): Promise<ResOutput> => {
    const {
      req,
      res,
    ...
    const { logErrors, respondErrors, allowFail, onError } =
      resolverConfig || {};
    ...
    const apiParamKeys = [ctx?.paramKeys, paramKeys || schemaParamKeys]
      .flat()
      .filter(Boolean)
      .join(' '); // prettier-ignore
    const query = {
      ...nextSsrContext?.query,
      ...(req as NextApiRequest)?.query,
    };
    const params = {
      ...restParams,
      ...nextSsrContext?.params,
      ...context,
      ...ctx?.params,
    }; // @ts-ignore
    const cookies =
      nextSsrContext?.req?.cookies || req?.cookies || ctx?.cookies;
    const relatedArgs = apiParamKeys
      ? getApiParams(apiParamKeys, { query, params, body, args, context })
      : {}; // prettier-ignore
