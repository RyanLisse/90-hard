import type { Metadata, SCHEMA_TYPE } from './index';
...
export type SchemaPluginMap<S extends Record<string, Metadata>> =
  SchemaTypeMap & {
...
>(
  ...
  if (typeof schemaMeta.introspect === 'function') {
    throw new Error(
      "createSchemaPlugin() was passed a zod schema instead of it's metadata, please use .introspect() on the schema instead"
    );
  }
  if (!schemaMeta.schema) {
    throw new Error(
      'createSchemaPlugin() was passed a schema without a schema property'
    );
  }
  // Map schema to new structure
  const mappedSchema = Object.entries(schemaMeta.schema!).reduce(
    (result, [schemaKey, fieldMeta]) => {
      ...
      if (schemaTypeMap[baseType]) {
        mappedSchemaBuilder = schemaTypeMap[baseType];
      }
      if (schemaTypeMap[zodType]) {
        mappedSchemaBuilder = schemaTypeMap[zodType];
      }
      if (schemaTypeMap[schemaKey]) {
        mappedSchemaBuilder = schemaTypeMap[schemaKey];
      }
      // Skip if no builder was found
      if (!mappedSchemaBuilder) {
        return result;
      }
  ...
  return mappedSchema as {
    [K in keyof M]: SchemaPluginMap<M>[K] extends (
      schemaKey: string,
      fieldMeta: Metadata
    ) => infer R
      ? R
