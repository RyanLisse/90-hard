...
import type { Meta$Schema, Metadata, ZodSchema } from './index';
...
export const createResolverDefinition = (resolverConfig: ResolverConfig) => {
  ...
  const areArgsRequired =
    hasArgs && !argsMeta?.isNullable && !argsMeta?.isOptional;
  const areArgsOptional =
    !areArgsRequired ||
    Object.values(argsMeta?.schema!).every(
      (arg) => arg.isOptional || arg.isNullable
    ); // prettier-ignore
...
export const createSchemaDefinition = (
  ...
  const schemaDefinitionBuilder = (graphqlType: string) => {
    return (schemaKey: string, fieldMeta: Meta$Schema | Metadata<any>) => {
      ...
      if (graphqlType === 'Schema' && fieldMeta.schema) {
        ...
        schemaDefinitions = [
          ...schemaDefinitions,
          ...createSchemaDefinition(fieldMeta),
        ];
      ...
      if (graphqlType === 'Array') {
        ...
        const primitiveElement =
          SCHEMA_PRIMITIVES_LOOKUP[
            fieldMeta.schema.baseType as keyof typeof SCHEMA_PRIMITIVES_LOOKUP
          ]; // prettier-ignore
        const arrayElementType =
          primitiveElement ||
          normalizeSchemaName(fieldMeta.schema.name!, prefix); // prettier-ignore
        // If the array is a schema, we should add that schema's definitions as well
        if (!primitiveElement && fieldMeta.schema.schema) {
          schemaDefinitions = [
            ...schemaDefinitions,
            ...createSchemaDefinition(
              fieldMeta.schema as unknown as Meta$Schema,
              prefix
            ),
          ];
        }
  ...
  if (finalSchemaDef.includes('undefined')) {
    warnOnce(
      `-!- Skipping schema definition for ${finalSchemaName} due to undefined fields`,
      {
        schema,
        prefix,
        schemaDefinitions,
...
export const createDataDefinitions = (resolverConfigs: ResolverConfig[]) => {
  ...
  const dataDefinitions = resolverConfigs.reduce(
    (acc, { argSchema, resSchema }) => {
      let argDefinitions = createSchemaDefinition(
        argSchema.introspect() as Meta$Schema,
        'input'
      );
      let resDefinitions = createSchemaDefinition(
        resSchema.introspect() as Meta$Schema,
        'type'
      );
...
export const createGraphSchemaDefs = (resolvers: ResolversMapType) => {
  ...
  if (isEmpty(resolvers)) {
    warnOnce(
      '-i- No resolvers provided to createGraphSchemaDefs(), no schema definition to generate'
    );
    return {
      queryResolvers: {},
      mutationResolvers: {},
  ...
  const rebuildFromConfig = (
    handlers: any,
    { resolverName, resolver }: ResolverConfig
  ) => ({
    ...handlers,
    [resolverName]: resolver,
  });
  // Figure out the mutation definitions
  const mutationConfigs = resolverConfigs.filter(
    (config) => !!config.isMutation
  );
  const mutationResolvers = mutationConfigs.reduce(rebuildFromConfig, {});
  const mutationDefs = mutationConfigs
    .map(createResolverDefinition)
    .filter(Boolean);
  const hasMutations = mutationDefs.length > 0;
  const mutationDef = hasMutations
    ? `type Mutation {\n    ${mutationDefs.join('\n    ')}\n}`
    : ''; // prettier-ignore
  ...
  const queryDef = hasQueries
    ? `type Query {\n    ${queryDefs.join('\n    ')}\n}`
    : '';
  // Figure out the data definitions
  const dataSchemaDefs = Array.from(
    new Set(createDataDefinitions(resolverConfigs))
  );
  // Combine all definitions
  const allSchemaDefs = [...dataSchemaDefs, queryDef, mutationDef].filter(
    Boolean
  );
