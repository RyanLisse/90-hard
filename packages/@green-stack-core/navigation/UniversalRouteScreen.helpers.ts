...
> = (args: ARGS, context?: QueryConfig<TadaDocumentNode>) => Promise<RES>;
...
export type QueryBridgeConfig<
  ...
  FetcherToProps extends (
    data: Awaited<ReturnType<QueryFn<ARGS, RES>>>
  ) => unknown = (
    data: Awaited<ReturnType<QueryFn<ARGS, RES>>>
  ) => Awaited<ReturnType<QueryFn<ARGS, RES>>>,
  ParamsToQueryKey extends (
    routeParams: ExtractRouteParams<FetcherArgs>
  ) => QueryKey = (routeParams: ExtractRouteParams<FetcherArgs>) => QueryKey,
  ParamsToQueryInput extends (
    routeParams: ExtractRouteParams<FetcherArgs>
  ) => FetcherArgs = (
    routeParams: ExtractRouteParams<FetcherArgs>
  ) => FetcherArgs,
...
> = Prettify<
  ReturnType<Exclude<QueryBridge['fetcherDataToProps'], undefined>> & {
    ...
    refetchInitialData?: () => Promise<{
      data: ReturnType<Exclude<QueryBridge['fetcherDataToProps'], undefined>>;
    }>;
  }
>;

/** --- createQueryBridge() -------------------------------------------------------------------- */
/** -i- Util to create a typed bridge between a fetcher and a route's props */ // @ts-expect-error
export const createQueryBridge = <
  ...
  FetcherData = Awaited<
    ReturnType<
      QueryBridgeConfig<ARGS, RES, QueryFn<ARGS, RES>>['routeDataFetcher']
    >
  >,
  FetcherToProps extends (
    fetcherData: Awaited<ReturnType<QueryFn<ARGS, RES>>>
  ) => unknown = (
    fetcherData: Awaited<ReturnType<QueryFn<ARGS, RES>>>
  ) => Awaited<ReturnType<QueryFn<ARGS, RES>>>,
  ParamsToQueryKey extends (
...
>(
  ...
  const fetcherDataToProps =
    ...
    ((fetcherData: FetcherData) => fetcherData);
  return {
    ...queryBridge,
...
export const DEFAULT_QUERY_BRIDGE = {
  routeParamsToQueryKey: (routeParams: Record<string, unknown>) => [
    JSON.stringify(routeParams),
  ],
  routeParamsToQueryInput: (routeParams: Record<string, unknown>) =>
    routeParams,
