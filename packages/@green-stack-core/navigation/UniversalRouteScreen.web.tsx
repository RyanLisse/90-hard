...
import {
  DEFAULT_QUERY_BRIDGE,
  type QueryFn,
  type UniversalRouteProps,
} from './UniversalRouteScreen.helpers';
...
const getSSRData = () => {
  ...
  const ssrData = ssrDataText
    ? (JSON.parse(ssrDataText) as Record<string, any>)
    : null;
  return ssrData;
};
...
const getDehydratedSSRState = () => {
  ...
  const ssrHydrationState = ssrHydrationStateText
    ? (JSON.parse(ssrHydrationStateText) as Record<string, any>)
    : null;
...
>(
  ...
  const { routeParamsToQueryKey, routeParamsToQueryInput, routeDataFetcher } =
    queryBridge;
  const fetcherDataToProps =
    queryBridge.fetcherDataToProps ||
    ((data: Awaited<ReturnType<QueryFn<ARGS, RES>>>) => data);
  ...
  // State
  const [hydratedData, setHydratedData] = useState<Record<string, any> | null>(
    null
  );
  const [hydratedQueries, setHydratedQueries] = useState<Record<
    string,
    any
  > | null>(null);
  ...
  const shouldAddContext =
    !isEmpty(requestContext) &&
    !!routeDataFetcher?.isUniversalQuery &&
    !isBrowser;
  ...
  useEffect(() => {
    ...
    if (hydratedQueyClientState) {
      setHydratedQueries(hydratedQueyClientState); // Save the hydrated state to state, removing the hydrated state from the DOM
    }
  }, []);
  ...
  if (isBrowser) {
    ...
    const { data: fetcherData, ...query } = useQuery({
      ...
      initialData: shouldRefetchOnMount
        ? undefined
        : {
    ...
    const routeDataProps = fetcherDataToProps(fetcherData as any) as Record<
      string,
      unknown
    >; // prettier-ignore
    ...
    const refetchInitialData = async () => {
      ...
      const regeneratedProps = fetcherDataToProps(
    ...
    return (
      <HydrationBoundary state={hydrationState}>
        ...
        {renderHydrationData && (
          <div
            data-ssr={JSON.stringify(hydrationState)}
