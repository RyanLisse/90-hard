...
const collectResolvers = () => {
  try {
    ...
    const featureAPIRoutes = globRel(
      '../../features/**/routes/api/**/route.ts'
    ).filter(excludeDirs); // prettier-ignore
    const packageAPIRoutes = globRel(
    ...
    const resolverRegistry = allAPIRoutes.reduce((acc, resolverPath) => {
      ...
      const [packageParts, routeParts] = resolverPath.split('/routes') as [
        string,
        string,
      ];
      ...
      const exportsSchemaResolver = pathContents.includes(
        'createGraphResolver'
      );
      const isCommented = pathContents.includes(
        '// export const graphResolver'
      );
      const exportsGraphQLResolver =
        pathContents.includes('export const graphResolver') && !isCommented; // prettier-ignore
      if (!(exportsSchemaResolver && exportsGraphQLResolver)) {
        return acc;
      }

      // Skip files that have opt-out patterns
      if (hasOptOutPatterns(pathContents)) {
        return acc;
      }
      ...
      const graphResolverLine = lines.find((line) => {
        return line.includes(
          'export const graphResolver = createGraphResolver'
        );
      });
      const resolverName = findTargetString(
        graphResolverLine!,
        'createGraphResolver($target$)'
      );
      if (!resolverName) {
        return acc;
      }
