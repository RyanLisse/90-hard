...
const collectGenerators = () => {
  try {
    ...
    const featureGenerators = globRel(
      '../../features/**/generators/*.ts'
    ).filter(excludeDirs);
    const packageGenerators = globRel(
      '../../packages/**/generators/*.ts'
    ).filter(excludeDirs);
    const rawGenerators = [...featureGenerators, ...packageGenerators].filter(
      (path) => !path.includes('.d.ts')
    ); // prettier-ignore
    ...
    const generatorRegistry = allGenerators.reduce((acc, generatorPath) => {
      ...
      const [packageParts, generatorParts] = generatorPath.split(
        '/generators'
      ) as [string, string];
      ...
      const exportLines = pathContents
        .split('\n')
        .filter((line) => line.includes('export') && !line.includes('${'));
      const generatorExportLine = exportLines.find(
        (line) =>
          line.includes('export const register') && line.includes('Generator')
      );
      if (!(usesPlopTypes && usesTurboGenerators && generatorExportLine)) {
        return acc;
      }

      // Extract the generator name
      const generatorName = findTargetString(
        generatorExportLine,
        'export const $target$ ='
      );
      return generatorName
        ? `${acc}export { ${generatorName} } from '${importPath}'\n`
        : acc;
    }, genMsg);
