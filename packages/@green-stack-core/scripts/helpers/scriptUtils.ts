...
export const matchMethods = (
  methods: HINTED_METHODS[],
  method: HINTED_METHODS
) => {
...
export const globRel = (globPath: string) => {
  ...
  const [absoluteRootDir] = import.meta.dirname
    .replaceAll('\\', '/')
    .split('packages/@green-stack-core/scripts');
  // Replace the root app dir with the folder level
  return allMatches.map((match) =>
    match.replace(absoluteRootDir, folderLevel).replaceAll('\\', '/')
  );
};
...
export const createDivider = (title: string, isDocDivider = false) => {
  const baseTemplate = isDocDivider
    ? `/** --- ${title}  */`
    : `/* --- ${title}  */`;
...
export const maybeImport = (
  path: string,
  onError: 'logErrors' | 'throwErrors' | 'ignoreErrors' = 'ignoreErrors'
) => {
  try {
    const importedModule = require(path);
    return importedModule || {};
  } catch (error) {
    if (onError === 'logErrors') {
    }
...
export const parseWorkspaces = (
  ...
  const appConfigPaths = includeApps
    ? globRel(`${folderLevel}apps/**/package.json`).filter(excludeModules)
    : []; // prettier-ignore
  const packageConfigPaths = globRel(
    `${folderLevel}packages/**/package.json`
  ).filter(excludeModules); // prettier-ignore
  const featureConfigPaths = globRel(
    `${folderLevel}features/**/package.json`
  ).filter(excludeModules); // prettier-ignore
  ...
  const workspaceImports = packageJSONPaths.reduce((acc, packageJsonPath) => {
    ...
    const workspaceMatcher = packageJsonPath
      .replace(`${folderLevel}`, '')
  ...
  const workspacePathsMap = Object.entries(workspaceImports).reduce(
    (acc, [wsPath, pkgName]) => {
  ...
  const sortByOrder = (a: string, b: string) =>
    a.includes('packages') && !b.includes('packages') ? -1 : 1; // prettier-ignore
  const workspacePaths = Object.keys(workspaceImports).sort(sortByOrder);
  const workspacePackages = workspacePaths.map(
    (path) => workspaceImports[path]
  );
...
export const opt = (option: string, greyOut = false) => {
  const renderOption = (label: string) =>
    greyOut ? a.muted(label) : a.bold(label);
...
export const getWorkspaceOptions = (
  ...
  return workspaceOptions.reduce((options, workspacePath) => {
    ...
    const skippedHelpers = [
      !includeGreenStack && 'green-stack',
      'registries',
    ].filter(Boolean) as string[];
    if (skippedHelpers.some((helper) => workspaceName.includes(helper))) {
      return options;
    }
    // Add the workspace option if not excluded or if it matches the filter
    const isFiltered = filter.length
      ? filter.some((f) => workspaceOption.includes(f))
      : true;
    const isExcluded = exclude.length
      ? exclude.some((e) => workspaceOption.includes(e))
      : false;
    ...
    if (!skipFormatting) {
      workspaceOption = opt(workspaceOption);
    }
...
export const getAvailableSchemas = (
  ...
  const packageSchemaPaths = globRel(
    `${folderLevel}packages/**/schemas/[A-Z]*.ts`
  ).filter(excludeModules);
  const featureSchemaPaths = globRel(
    `${folderLevel}features/**/schemas/[A-Z]*.ts`
  ).filter(excludeModules);
  const allSchemaPaths = [...packageSchemaPaths, ...featureSchemaPaths].filter(
    (pth) => {
      return ![
        '@green-stack-core/schemas/',
        'createSchema',
        '.bridge',
        '.resolver',
        '.enum',
      ].some((excluded) => pth.includes(excluded));
    }
  );
  ...
  const availableSchemas = allSchemaPaths.reduce((acc, schemaPath) => {
    ...
    const workspacePath = schemaPath
      .split('/schemas/')?.[0]
      ?.replace(`${folderLevel}`, '');
    const workspaceName = workspaceImports[workspacePath];
    ...
    if (!includeOptOut && hasOptOutPatterns(fileContents)) {
      return acc;
    }
    ...
    const isDefaultExport = fileContents.includes(
...
export const getAvailableDataBridges = (
  ...
  const packageBridgePaths = globRel(
    `${folderLevel}packages/**/resolvers/**.bridge.ts`
  ).filter(excludeModules);
  const featureBridgePaths = globRel(
    `${folderLevel}features/**/resolvers/**.bridge.ts`
  ).filter(excludeModules);
  const allDataBridgePaths = [
    ...packageBridgePaths,
    ...featureBridgePaths,
  ].filter((pth) => !pth.includes('createDataBridge'));

  // Get path of all Fetchers
  const packageQueryPaths = globRel(
    `${folderLevel}packages/**/resolvers/**.query.ts`
  ).filter(excludeModules);
  const featureQueryPaths = globRel(
    `${folderLevel}features/**/resolvers/**.query.ts`
  ).filter(excludeModules);
  const packageMutationPaths = globRel(
    `${folderLevel}packages/**/resolvers/**.mutation.ts`
  ).filter(excludeModules);
  const featureMutationPaths = globRel(
    `${folderLevel}features/**/resolvers/**.mutation.ts`
  ).filter(excludeModules);
  const allFetcherPaths = [
    ...packageQueryPaths,
    ...featureQueryPaths,
    ...packageMutationPaths,
    ...featureMutationPaths,
  ];

  // Build lookup of available fetchers
  const availableFetchers = allFetcherPaths.reduce(
    (acc, fetcherPath) => {
      // Skip files that have opt-out patterns
      const fetcherFileContents = fs.readFileSync(fetcherPath, 'utf8');
      if (!includeOptOut && hasOptOutPatterns(fetcherFileContents)) {
        return acc;
      }

      // Figure out fetcher name and type
      const [resolverName, fetcherType] = fetcherPath
        .split('/')
        .pop()
        ?.split('.') as [string, 'query' | 'mutation'];
      const fetcherName = fetcherFileContents.match(
        /export const (\w+) = bridgedFetcher\({/
      )?.[1];
      ...
      return {
        ...
        [resolverName]: {
          resolverName,
          fetcherName:
            fetcherName || `${resolverName}${uppercaseFirstChar(fetcherType)}`,
          fetcherType,
        },
      };
    },
  ...
  const availableDataBridges = allDataBridgePaths.reduce((acc, bridgePath) => {
    ...
    if (!includeOptOut && hasOptOutPatterns(bridgeFileContents)) {
      return acc;
    }

    // Figure out the bridge name and contents
    const bridgeName = bridgePath
      .split('/')
      .pop()
      ?.replace('.bridge.ts', 'Bridge');
    const workspacePath = bridgePath
      .split('/resolvers/')[0]
      ?.replace(`${folderLevel}`, '');
    const workspaceName = workspaceImports[workspacePath];

    // Stop if the bridge is not exported of not found due to name not matching
    const isNamedExport = bridgeFileContents.includes(
      `export const ${bridgeName}`
    );
    const isDefaultExport = bridgeFileContents.includes(
    ...
    const isCallingCreateDataBridge =
      bridgeFileContents.includes('createDataBridge(');
    const resolverName = bridgeFileContents.match(/resolverName: '(\w+)'/)?.[1];
    if (!(isCallingCreateDataBridge && resolverName)) {
      return acc;
    }
    ...
    const allowedMethodsLine = bridgeFileContents.match(
      /allowedMethods: \[(.+)\]/
    )?.[1];
    const allowedMethods = allowedMethodsLine
      ?.split(',')
      .map((method) => method.replaceAll(`'`, '').trim());
    const hasGraphResolver = allowedMethodsLine?.includes('GRAPHQL');
    if (!(allowNonGraphql || hasGraphResolver)) {
      return acc;
    }
    let resolverType = bridgeFileContents.match(/resolverType: '(\w+)'/)?.[1] as
      | 'query'
      | 'mutation';
    if (!resolverType) {
      resolverType = relatedFetcher?.fetcherType;
    }
    const isMutation =
      bridgeFileContents.includes('isMutation: true') ||
      allowedMethods?.some((method) =>
        ['POST', 'PUT', 'DELETE'].includes(method)
      );
    const isQuery =
      bridgeFileContents.includes('isMutation: false') ||
      allowedMethods?.some((method) => ['GET'].includes(method));
    if (!resolverType && isMutation) {
      resolverType = 'mutation';
    }
    if (!resolverType && isQuery) {
      resolverType = 'query';
    }
    if (filterType && resolverType !== filterType) {
      return acc;
    }

    // Determine operationType based on allowed methods or resolverName
    let operationType = '' as
      | 'add'
      | 'update'
      | 'delete'
      | 'get'
    ...
    if (
      ...
      includesAny(resolvername, ['add', 'create', 'insert'])
    ) {
      operationType = 'add';
    } else if (
      allowedMethods?.includes('PUT') ||
      includesAny(resolvername, ['update', 'edit', 'modify'])
    ) {
      operationType = 'update';
    } else if (
      allowedMethods?.includes('DELETE') ||
      includesAny(resolvername, ['delete', 'remove'])
    ) {
      operationType = 'delete';
    } else if (
      allowedMethods?.includes('GET') ||
      includesAny(resolvername, ['get', 'fetch', 'retrieve'])
    ) {
      operationType = 'get';
    } else if (
      allowedMethods?.includes('LIST') ||
      includesAny(resolvername, ['list', 'all'])
    ) {
      operationType = 'list';
    } else if (
      allowedMethods?.includes('SEARCH') ||
      includesAny(resolvername, ['search', 'find'])
    ) {
    ...
    const outputSchemaName =
      bridgeFileContents.match(/outputSchema: (\w+)/)?.[1];
    ...
    const bridgeEntry = {
      ...
      operationType,
      fetcherName:
        relatedFetcher?.fetcherName ||
        `${resolverName}${uppercaseFirstChar(resolverType)}`,
      inputSchemaName,
      outputSchemaName,
      allowedMethods,
      isNamedExport,
...
export const readImportAliases = (folderLevel = '../../') => {
  return Object.entries(tsconfig.compilerOptions.paths).reduce(
    (acc, [alias, [path]]) => {
      ...
      const workspaceEntry = Object.entries(workspaceImports).find(
        ([pathKey]) => {
...
export const swapImportAlias = (importPath: string, folderLevel = '../../') => {
  ...
  const aliasMatch = Object.keys(importAliases).find((alias) =>
    importPath.includes(alias)
  );
  return aliasMatch
    ? importPath.replace(aliasMatch, importAliases[aliasMatch])
    : importPath;
};
