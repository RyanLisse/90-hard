...
export type CheckboxPrompts<CHOICES extends readonly string[]> =
  CommonPrompt<'checkbox'> & {
    choices: CHOICES;
  };

export type ListPrompts<CHOICES extends readonly string[]> =
  CommonPrompt<'list'> & {
    choices: CHOICES;
  };

export type AutocompletePrompts<CHOICES extends readonly string[]> =
  CommonPrompt<'autocomplete'> & {
    choices: CHOICES;
  };
...
export type FullPrompt = {
  ...
  choices?:
    ...
    | Record<string, string>
    | { name: string; value: string }[]
    | ((answers: any$Unknown) => string[] | { name: string; value: string }[]);
  source?: (
    answers: any$Unknown,
    input: string
  ) => Promise<string[] | { name: string; value: string }[]>;
  default?: any$Unknown;
...
export const createOptionLookup = <
  T extends Record<K, any$Unknown>,
  K extends keyof T,
>(
  options: T[],
  nameKey: K,
...
export const createAutocompleteSource = (
  ...
  return (_answersSoFar: any$Todo, input = '') => {
    const filteredOptions = options.filter((option) => {
      let checkedString = option as string;
      if (typeof option !== 'string') {
        checkedString = `${option.name} ${option.value}`;
      }
...
export const createPrompts = <
  ...
  VALUES = {
    [K in keyof PROMPTS]: PromptValue<
      PROMPTS[K]['type'],
      ExtractChoices<PROMPTS[K]['choices']>
    >;
  }, // @ts-ignore
  POTENTIAL_REFINEMENTS = { [K in keyof VALUES]: PromptRefinement<K, VALUES> },
  REFINEMENTS extends
    Partial<POTENTIAL_REFINEMENTS> = Partial<POTENTIAL_REFINEMENTS>,
