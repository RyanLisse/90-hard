...
const createExportsFile = (ctx: { exports: string }) =>
  [
...
const checkWorkspaces = async (isDeepCheck = true) => {
  try {
    ...
    const [envFilePath] = [
      hasMainEnv && mainEnvFilePath,
      hasLocalEnv && localEnvFilePath,
    ].filter(Boolean) as string[]; // prettier-ignore

    const { config: dotenvConfig } = await import('dotenv');
    if (isDev && envFilePath) {
      dotenvConfig({ path: envFilePath });
    }

    // Load main tsconfig.json
    const appCoreTsConfig = JSON.parse(
      fs.readFileSync('../../features/@app-core/tsconfig.json', 'utf8')
    ); // prettier-ignore
    const typescriptAliases = Object.keys(
      appCoreTsConfig.compilerOptions.paths
    ...
    const allWsFiles = [...featureFiles, ...packageFiles].filter(
      excludeModules
    );
    ...
    const {
      ...
      workspacePaths,
      workspacePackages,
    } = parseWorkspaces(); // prettier-ignore

    // Figure out package versions
    const expoPackageConfig = JSON.parse(
      fs.readFileSync('../../apps/expo/package.json', 'utf8')
    );
    const nextPackageConfig = JSON.parse(
      fs.readFileSync('../../apps/next/package.json', 'utf8')
    );
    const allPackageConfigs = [
      expoPackageConfig,
      nextPackageConfig,
      ...Object.values(workspaceConfigs),
    ]; // prettier-ignore
    ...
    await Promise.all(
      workspacePaths.map(async (workspacePath) => {
        ...
        if (isDeepCheck) {
          ...
          const workspaceFiles = allWsFiles.filter((filePath) =>
            filePath.includes(`${workspacePath}/`)
          );
          const fileContents = workspaceFiles.map((filePath) =>
            fs.readFileSync(filePath, 'utf8')
          );
          const allLinesOfCode = fileContents.flatMap((content) =>
            content.split('\n')
          );

          // Rebuild the related workspaces list
          const filterByRelevancy = (line: string) =>
            line.includes(` from '`) && !line.includes('./');
          const allImportLines = allLinesOfCode.filter(filterByRelevancy);
          const allImportPaths = allImportLines.map(
            (line) => line.split(` from '`)[1].split(`'`)[0]
          );
          const containsWorkspaces = (wsPkg: string) =>
            allImportPaths.some((path) => path.includes(`${wsPkg}/`));
          const newRelatedWorkspaces =
            workspacePackages.filter(containsWorkspaces);

          // List all the packages being used in that workspace
          const workspaceDeps = allImportPaths.reduce(
            (acc, importPath) => {
              ...
              if (typescriptAliases.includes(packageName)) {
                return acc;
              }
          ...
          const allProcessEnvLines = allLinesOfCode.filter((line) =>
            line.includes('process.env.')
          );
          const allProcessEnvVars = allProcessEnvLines.map(
            (line) => line.match(/process\.env\.([A-Z0-9_]+)/)?.[1]
          ); // prettier-ignore
          const filterEnvs = (envVar?: string) => {
            if (shouldSkip) {
              return false; // Don't check @green-stack packages
            }
          ...
          const existingRelations =
            packageJSON?.stackConfig?.relatedWorkspaces || [];
          const prevRequiredEnvVars =
            packageJSON?.stackConfig?.requiredEnvVars || [];
          const relatedWorkspaces = Array.from(
            new Set([...existingRelations, ...newRelatedWorkspaces])
          ); // prettier-ignore
          const requiredEnvVars = Array.from(
            new Set([...prevRequiredEnvVars, ...newRequiredEnvVars])
          ); // prettier-ignore
          ...
          const missingDeps = Object.keys(workspaceDeps).filter(
            (dep) => !packageDeps.includes(dep)
          );
          const hasMissingDeps = missingDeps.length > 0;

          // Save the updated package.json?
          const hasChangedRelations =
            existingRelations.join('-') !== relatedWorkspaces.join('-');
          const hasChangedEnvVars =
            prevRequiredEnvVars.join('-') !== requiredEnvVars.join('-');
          const hasChanged =
            !shouldSkip && isDev && (hasChangedRelations || hasChangedEnvVars);
          const shouldChange = isDeepCheck && (hasChanged || hasMissingDeps);
          if (shouldChange) {
            if (!packageJSON.stackConfig) {
              packageJSON.stackConfig = { relatedWorkspaces, requiredEnvVars }; // prettier-ignore
            }
            packageJSON.dependencies = {
              ...packageJSON.dependencies,
              ...workspaceDeps,
            ...
            fs.writeFileSync(
              ...
              `${JSON.stringify(packageJSON, null, __tabSize)}`
            );
          }
        }
        ...
        const getEnvVar = (rawEnvVar: string) => {
          ...
          return (
            process.env[envVar] ||
            process.env[`NEXT_PUBLIC_${envVar}`] ||
            process.env[`EXPO_PUBLIC_${envVar}`]
          );
        };
        const missingEnvVars = requiredEnvVars.filter((envVar: string) =>
          isEmpty(getEnvVar(envVar))
        );
        ...
        const missingWorkspaces = relatedWorkspaces.filter(
          (ws: string) => !workspaceIdentifiers.includes(ws)
        );
        ...
        workspaceResolutions = {
          ...workspaceResolutions,
          ...stackConfig?.resolutions,
        };

        // Warn of any missing env vars or related workspaces
        if (missingEnvVars.length || missingWorkspaces.length) {
        }
    ...
    const transpiledWorkspacesPath =
      '../../packages/@registries/transpiledWorkspaces.generated.js';
    const transpiledWorkspaces = createExportsFile({
      exports: JSON.stringify(workspacePackages, null, 4),
    }); // template.replace('{{exports}}', JSON.stringify(workspacePackages, null, 4)) // prettier-ignore
    ...
    const workspaceResolutionsPath =
      '../../packages/@registries/workspaceResolutions.generated.js';
    const workspaceResolutionsFile = createExportsFile({
      exports: JSON.stringify(workspaceResolutions, null, 4),
    }); // template.replace('{{exports}}', JSON.stringify(workspaceResolutions, null, 4)) // prettier-ignore
    fs.writeFileSync(
      workspaceResolutionsPath,
      workspaceResolutionsFile,
      'utf8'
    );
  } catch (_err) {
    process.exit(1);
