...
const collectDrivers = () => {
  try {
    ...
    const driverConfig = allDriverPaths.reduce((acc, driverFilePath) => {
      ...
      const isValidDriver = driverFileContents.includes(
      ...
      const driverFileModuleName = driverFilename
        .replace('.tsx', '')
        .replace('.ts', '');
      const [driverName, driverType] = driverFileModuleName.split('.');
      const driverImportPath = `${driverWorkspace}/drivers/${driverFilename}`;
      const driverKey =
        driverName === 'mock' ? `mock-${driverType}` : driverName;
      const driverTypeKey =
        driverType.length <= 2 ? driverType.toUpperCase() : driverType;
      const driverEnumKey =
        driverName === 'mock' ? `mock${driverTypeKey}` : driverName;
      ...
      return {
        ...
        driverConfigTypeLines: addSetItem(
          acc.driverConfigTypeLines,
          driverConfigTypeLine
        ),
        drivers: {
          [driverType]: {
            driverType,
    ...
    const configTypeSections = Object.values(driverConfig.drivers)
      .map((driverData) => {
        const { driverType, driverConfigTypeEnumLines } = driverData;
        return createDriverTypeEntries({
          driverType,
          driverConfigTypeEnumLines,
        });
      })
      .join('\n');
    ...
    fs.writeFileSync(
      '../../packages/@registries/drivers.config.ts',
      driverConfigContent
    );

    // Build drivers.generated.ts subtype exports
    const driverTypeExports = Object.values(driverConfig.drivers)
      .map((driverData) => {
        const { driverTypeKey, driverTypeExportKey, driverTypeEntryLines } =
          driverData;
