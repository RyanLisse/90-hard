...
export const parseNativewindStyles = (style: any) => {
  return Object.entries(style || {}).reduce(
    (acc, [key, value]) => {
      ...
      if (['mask', 'childClassNames'].includes(key)) {
        return acc;
      }
      ...
      if (Number.isInteger(Number(key))) {
        ...
        if (isCSS) {
          ...
          const { $$css: _, ...classNameObjects } = value as Record<
            string,
            unknown
          >;
          const className = [
            acc.nativeWindClassName,
            ...Object.values(classNameObjects),
          ]
            .filter(Boolean)
        ...
        if (Array.isArray(value)) {
          ...
          const flattenedStyles = value.reduce(
            (acc, val) => ({ ...acc, ...val }),
            {}
          );
          return {
            ...acc,
            nativeWindStyles: { ...acc.nativeWindStyles, ...flattenedStyles },
          }; // prettier-ignore
        }
